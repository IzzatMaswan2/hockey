    ...
    <div class="container-fluid" style="height: 90%; padding: 0;">
        <div class="row">
            ...
            <div class="col-9 mt-5">
                <h1>Group Match</h1>
                <h2 style="display: inline-block; margin: 0;">Ongoing Matches</h2>
                <a href="{{ url('/match-score/knockout') }}" style="float: right;">
                    <button style="
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if(!empty($match->teamA->logoURL) && Storage::disk('public')->exists($match->teamA->logoURL))
                                    <img src="{{ asset('storage/' . $match->teamA->logoURL) }}" alt="{{ $match->teamA->Name }}" class="team-logo" style="width: 40px; height: 40px;">
                                @endif
                                {{ $match->teamA->Name }} vs
                                @if(!empty($match->teamB->logoURL) && Storage::disk('public')->exists($match->teamB->logoURL))
                                    <img src="{{ asset('storage/' . $match->teamB->logoURL) }}" alt="{{ $match->teamB->Name }}" class="team-logo" style="width: 40px; height: 40px;">
                                @endif
                                {{ $match->teamB->Name }}
                            </td>
                            ...
                            <td>
                                @if($match->TeamAID == $teamId)
                                    ...
                                    @if($approval && $approval->managerA_approved >= 1)
                                        <div>Pending</div>
                                    @else
                                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#scoreModal" onclick="document.getElementById('modal_match_id').value = {{ $match->Match_groupID }}">
                                            Submit Score
                                        </button>
                                    @endif
                                @elseif($match->TeamBID == $teamId)
                                    @php
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form action="{{ route('match-score.submit') }}" method="POST" class="modal-content">
                                                @csrf
                                                <input type="hidden" name="match_id" id="modal_match_id"> <!-- Hidden input for match_id -->
                                                
                                                <div class="mb-3">
                                                    <label for="score_a" class="form-label">{{$match->teamA->Name}} Score:</label>
                                                    <input type="number" id="score_a" name="score_a" class="form-control" required>
                                                </div>
                                                <div class="mb-3">
                                                    <label for="score_b" class="form-label">{{$match->teamB->Name}} Score:</label>
                        ...
                        @foreach ($upcomingmatches as $match)
                            <tr>
                                ...
                                <td>
                                    <div>
                                        @if(!empty($match->teamA->logoURL) && Storage::disk('public')->exists($match->teamA->logoURL))
                                            <img src="{{ asset('storage/' . $match->teamA->logoURL) }}" alt="{{ $match->teamA->Name }}" class="team-logo me-2" style="width: 40px; height: 40px;">
                                        @endif
                                        {{ $match->teamA->Name ?? 'Unknown Team A' }}
                                        vs
                                        @if(!empty($match->teamB->logoURL) && Storage::disk('public')->exists($match->teamB->logoURL))
                                            <img src="{{ asset('storage/' . $match->teamB->logoURL) }}" alt="{{ $match->teamB->Name }}" class="team-logo ms-2" style="width: 40px; height: 40px;">
                                        @endif
                                        {{ $match->teamB->Name ?? 'Unknown Team B' }}
                                    </div>
                                </td>
                                <td>{{ $match->Date }} & {{ $match->start_time }}</td>
                                <td>
                                    <!-- Button to open modal -->
                                    <button class="btn btn-info" data-bs-toggle="modal" data-bs-target="#matchDetailModal{{ $match->Match_groupID }}">
                                        See Details
                                    </button>
                                </td>
                            </tr>
                            ...
                            <div class="modal fade" id="matchDetailModal{{ $match->Match_groupID }}" tabindex="-1" aria-labelledby="matchDetailModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                        ...
                                        <div class="modal-body">
                                            <p><strong>Match ID:</strong> {{ $match->Match_groupID }}</p>
                                            <p><strong>Team A:</strong> 
                                                @if(!empty($match->teamA->logoURL) && Storage::disk('public')->exists($match->teamA->logoURL))
                                                    <img src="{{ asset('storage/' . $match->teamA->logoURL) }}" alt="{{ $match->teamA->Name }}" style="width: 40px; height: 40px;">
                                                @endif
                                                {{ $match->teamA->Name ?? 'Unknown Team A' }}
                                            </p>
                                            <p><strong>Team B:</strong> 
                                                @if(!empty($match->teamB->logoURL) && Storage::disk('public')->exists($match->teamB->logoURL))
                                                    <img src="{{ asset('storage/' . $match->teamB->logoURL) }}" alt="{{ $match->teamB->Name }}" style="width: 40px; height: 40px;">
                                                @endif
                                                {{ $match->teamB->Name ?? 'Unknown Team B' }}
                                            </p>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        @foreach ($endedmatches as $match)
                                    ...
                                    @if($match->match_status == 2)
                                        @if($match->TeamAID == $teamId)
                                            ...
                                            @if($approval)
                                                ...
                                                <button class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#approvalModal{{ $match->Match_groupID }}">
                                                    Approve Score
                                                </button>
                            ...
                            <!-- Approval Modal -->
                            <div class="modal fade" id="approvalModal{{ $match->Match_groupID }}" tabindex="-1" aria-labelledby="approvalModalLabel{{ $match->Match_groupID }}" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="approvalModalLabel{{ $match->Match_groupID }}">Approve/Reject Score for Match {{ $match->Match_groupID }}</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                                ...
                                                @if ($match->match_status == 1)
                                                    @if($match->TeamAID == $teamId)
                                                        ...
                                                        @if($approval && $approval->managerB_approved == 1)
                                                            <div class="alert alert-warning">
                                                                ...
                                                                Do you approve the score given by Team {{ $match->teamB->Name }} which is:
                                                            </div>
                                                            <div class="row">
                                                    ...
                                                    @elseif($match->TeamBID == $teamId)
                                                        ...
                                                        @if($approval && $approval->managerA_approved == 1)
                                                            <div class="alert alert-warning">
                                                                Both of the scores you submitted are not the same.
                                                                Do you approve the score given by Team {{ $match->teamA->Name }} which is:
                                                            </div>
                                                            <div class="row">
                                                                <div class="col text-center">
                                                                    @if(!empty($match->teamA->logoURL) && Storage::disk('public')->exists($match->teamA->logoURL))
